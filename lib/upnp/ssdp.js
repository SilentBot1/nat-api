...
class Ssdp extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._waitForBind().then(() => {
        ...
        const onDevice = (info, address) => {
          ...
          resolve({ device: new Device(info.location), address })
        }
  ...
  async destroy () {
    ...
    return Promise.allSettled(this.sockets.map(socket => socket.close()))
  }
}
