...
module.exports.connect = (gateway) => {
...
class Client extends EventEmitter {
  ...
  async portMapping (opts) {
    ...
    return await this._request(opcode, opts)
  }

  async portUnmapping (opts) {
    debug('Client#portUnmapping()')
    opts.ttl = 0
    return await this.portMapping(opts)
  }

  async externalIp () {
    debug('Client#externalIp()')
    return await this._request(OP_EXTERNAL_IP)
  }
  ...
  onMessage (msg, rinfo) {
    ...
    const cb = (err) => {
      ...
      if (err) {
        ...
        req.resolve.apply(this, arguments)
      }
